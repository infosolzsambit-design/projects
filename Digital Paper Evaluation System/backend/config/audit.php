...
return [
    ...
    */
    ...
    // Belt-and-braces global exclude, on top of each model's own $auditExclude —
    // these must never appear in old_values/new_values regardless of model.
    'exclude' => [
        'password',
    ...
    */
    ...
    | Left false in dev/production (App::runningInConsole() is also true for
    | the whole process during `php artisan test`, so this is switched on
    | via phpunit.xml purely so feature tests can verify automatic model
    | auditing — it has no effect on real HTTP requests either way).
    |
    */

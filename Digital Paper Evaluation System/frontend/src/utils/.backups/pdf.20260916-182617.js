...
export async function extractDocumentLinesViaOCR(pdfDoc, { scale = 3.5, onProgress } = {}) {
  ...
  // Tesseract's default page-segmentation mode tries to auto-detect the
  // page's own layout (columns, images, tables, ...) — on a bordered
  // question-table (every real paper this feature targets), that guesswork
  // frequently misreads whole rows as garbage, badly enough that entire
  // questions came back unrecognizable (verified directly against a real
  // scanned paper's own table page: switching this one setting took a page
  // from ~20% of its rows readable to essentially all of them, with no
  // measurable regression on a born-digital-quality scan). SINGLE_COLUMN
  // tells it "read this as one column of text blocks, top to bottom" —
  // still fine for a plain paragraph-only page, since that's a single
  // column too.

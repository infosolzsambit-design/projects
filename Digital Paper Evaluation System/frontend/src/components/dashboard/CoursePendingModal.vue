<script setup>
// "See All" behind AdminDashboardView.vue's own Course Wise Pending
// Evaluation card (shown there only once course_pending_has_more is
// true) — GET /dashboard/course-pending, the same query as that card's
// own capped one (most pending first), just without the limit. Same
// table markup as the card itself, just given a full modal's worth of
// scroll room.
...
async function fetchRows() {
  ...
  loadError.value = ''
  try {
    const res = await api.get('/dashboard/course-pending')
    rows.value = res.data.data.map((c) => ({ ...c, pendingPct: c.pending_pct }))
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not load course wise pending evaluation.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" @click.self="close">
    <div class="bg-white rounded-[24px] shadow-card w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden">
      <div class="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-soft">
        <div>
          <h2 class="text-lg font-bold text-black">Course Wise Pending Evaluation</h2>
          <p class="text-[13px] text-muted mt-0.5">{{ rows.length }} course{{ rows.length === 1 ? '' : 's' }} with pending sheets.</p>
        </div>
          ...
          <table class="w-full min-w-[380px] text-left text-[12.5px]">
            <thead>
              <tr class="bg-subject-header text-white text-[11px] font-medium">
                ...
                <th class="px-2.5 py-2 text-center rounded-tr-xl">Pending %</th>
              </tr>
            </thead>
            <tbody class="bg-white">
              <tr v-if="loading">
                <td colspan="5" class="px-2.5 py-10 text-center text-muted">Loading&hellip;</td>
              </tr>
              <tr v-else-if="!rows.length">
                <td colspan="5" class="px-2.5 py-10 text-center text-muted">Nothing pending — every course is fully evaluated.</td>
              </tr>

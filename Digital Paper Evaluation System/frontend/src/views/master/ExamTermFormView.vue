...
const examTermId = computed(() => route.params.id || null)
const isEdit = computed(() => !!examTermId.value)
...
// whatever was actually loaded for edit (captured once loadExamTerm()
...
async function loadExamTerm() {
  loading.value = true
  loadError.value = ''
  try {
    const res = await api.get(`/exam-terms/${examTermId.value}`)
    const examTerm = res.data.data
    form.name = examTerm.name
    initialForm = { ...form }
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not load this exam term.'
  } finally {
    loading.value = false
  }
}
...
function applyServerErrors(err) {
  ...
  formError.value = data?.message || 'Could not save exam term.'
}
...
async function submitForm() {
  ...
  try {
    const payload = { ...form }
    if (isEdit.value) {
      await api.put(`/exam-terms/${examTermId.value}`, payload)
    } else {
      await api.post('/exam-terms', payload)
    }
    router.push({ name: 'master-exam-terms' })
    toast.success(isEdit.value ? 'Exam term updated successfully.' : 'Exam term created successfully.')
  } catch (err) {
    ...
    <div class="max-w-[1100px] mx-auto">
      ...
      <div class="flex items-start justify-between gap-3 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-[20px] sm:text-[24px] font-semibold text-brand leading-tight">{{ isEdit ? 'Edit Exam Term' : 'Add Exam Term' }}</h1>
            <p class="mt-1 text-[13px] sm:text-sm text-muted">
              {{ isEdit ? "Update this exam term's details." : 'Create a new exam term for the system.' }}
            </p>
          </div>
        </div>
        <button
      ...
      <form v-else class="space-y-4" @submit.prevent="submitForm">
        <section class="bg-white rounded-[28px] shadow-card p-4 sm:p-5">
          <h2 class="text-[16px] sm:text-[18px] font-semibold text-gray-900 mb-1">Exam Term Details</h2>
        ...
        <div class="flex flex-wrap gap-3 justify-center sm:justify-end pb-2">
          ...
          <button
            ...
            :disabled="saving"
          >
            {{ saving ? 'Saving…' : isEdit ? 'Save Changes' : 'Create Exam Term' }}
          </button>
        </div>
      </form>
    </div>

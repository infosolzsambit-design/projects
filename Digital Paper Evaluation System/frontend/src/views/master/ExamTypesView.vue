<script setup>
// Clone of ExamTermsView.vue for the Exam Type master — same shape/
// endpoints, just /exam-types instead of /exam-terms, and now the same
// exam-type-* permission gating (list/add/edit/delete/status-change).
...
async function fetchExamTypes(page = 1) {
  loading.value = true
  loadError.value = ''
  try {
    ...
    const res = await api.get('/exam-types', { params })
    examTypes.value = res.data.data.items
    Object.assign(pagination, res.data.data.pagination)
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not load exam types.'
  } finally {
    loading.value = false
  }
}
...
async function toggleStatus(examType) {
  togglingId.value = examType.id
  const next = !examType.status
  try {
    await api.put(`/exam-types/${examType.id}`, { status: next })
    examType.status = next
    toast.success(`Exam type ${next ? 'activated' : 'deactivated'} successfully.`)
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not update exam type status.'
  } finally {
...
function goToCreate() {
  router.push({ name: 'master-exam-types-create' })
}

function goToEdit(examType) {
  router.push({ name: 'master-exam-types-edit', params: { id: examType.id } })
}

async function removeExamType(examType) {
  const confirmed = await confirmDialog({
    title: 'Delete Exam Type',
    message: `Delete exam type "${examType.name}"? This can be undone by an admin later.`,
    confirmText: 'Delete',
  })
  if (!confirmed) return
  deletingId.value = examType.id
  try {
    await api.delete(`/exam-types/${examType.id}`)
    if (examTypes.value.length === 1 && pagination.current_page > 1) {
      await fetchExamTypes(pagination.current_page - 1)
    } else {
      await fetchExamTypes(pagination.current_page)
    }
    toast.success('Exam type deleted successfully.')
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not delete exam type.'
  } finally {
...
onMounted(async () => {
  ...
  if (authStore.can('exam-type-list')) fetchExamTypes(1)
})
</script>

<template>
  <p v-if="!permissionChecked" class="text-center text-sm text-muted py-10">Loading&hellip;</p>
  <div v-else-if="!authStore.can('exam-type-list')" class="bg-white rounded-2xl shadow-panel p-10 text-center">
  ...
  <div v-else>
    ...
    <div class="bg-white rounded-2xl shadow-panel px-4 sm:px-5 py-2 mb-5 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        ...
        <span class="text-[13px] sm:text-sm text-gray-700">Exam Type List</span>
      </div>
      <div v-if="authStore.can('exam-type-add')" class="flex items-center gap-2">
        <button
          type="button"
    ...
    <section class="relative bg-subject-header rounded-t-2xl rounded-b-none px-4 sm:px-5 py-4 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div class="flex items-center gap-3 min-w-0">
        ...
        <h2 class="text-white text-[16px] sm:text-lg font-medium truncate">All Exam Types</h2>
      </div>
        ...
        <table class="w-full min-w-[560px] text-left">
          <thead>
            <tr class="bg-subject-header text-white text-[12px] font-medium">
              ...
              <th class="px-4 py-1.5 font-medium" v-if="authStore.can('exam-type-status-change')">Status</th>
              <th class="px-4 py-1.5 font-medium text-center rounded-tr-2xl" v-if="authStore.can('exam-type-edit') || authStore.can('exam-type-delete')">Action</th>
            </tr>
          </thead>
          <tbody class="bg-white">
            ...
            <tr v-else-if="!examTypes.length">
              <td colspan="4" class="px-4 py-8 text-center text-sm text-muted">No exam types found.</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-1 font-semibold">{{ examType.name }}</td>
              <td v-if="authStore.can('exam-type-status-change')" class="px-4 py-1" @click.stop>
                <button
                  type="button"
                  role="switch"
                  :aria-checked="examType.status"
                  :disabled="togglingId === examType.id"
                  :title="examType.status ? 'Click to deactivate' : 'Click to activate'"
                  ...
                  :class="examType.status ? 'bg-btn-gradient text-white justify-start pl-2.5 pr-6' : 'bg-gray-300 text-gray-600 justify-end pl-6 pr-2.5'"
                  @click="toggleStatus(examType)"
                >
                  <span>{{ examType.status ? 'Active' : 'Inactive' }}</span>
                  <span
                    class="absolute top-[5px] left-[5px] w-[14px] h-[14px] rounded-full bg-white shadow transition-transform duration-200"
                    :class="examType.status ? 'translate-x-[56px]' : 'translate-x-0'"
                  ></span>
                </button>
              </td>
              <td v-if="authStore.can('exam-type-edit') || authStore.can('exam-type-delete')" class="px-4 py-1 text-center relative" @click.stop>
                <RowActionMenu>
                  <button v-if="authStore.can('exam-type-edit')" type="button" class="w-full flex items-center gap-2 px-3.5 py-2 text-[13px] text-gray-700 hover:bg-soft hover:text-brand-blue transition-colors" @click="goToEdit(examType)">
                    Edit
                  </button>
                  <button
                    v-if="authStore.can('exam-type-delete')"
                    type="button"
                    ...
                    @click="removeExamType(examType)"
                  >
                    {{ deletingId === examType.id ? 'Deleting…' : 'Delete' }}
                  </button>
                </RowActionMenu>
              </td>
            </tr>

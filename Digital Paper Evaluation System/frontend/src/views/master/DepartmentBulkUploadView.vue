...
function downloadTemplate() {
  const sample = [{ Name: 'Demo Department', Code: 'DEMO' }]
  const worksheet = XLSX.utils.json_to_sheet(sample)
  const workbook = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Departments')
  XLSX.writeFile(workbook, 'department-bulk-upload-template.csv', { bookType: 'csv' })
}
...
async function runValidate() {
  ...
  modalError.value = ''
  try {
    const res = await api.post('/departments/bulk/validate', { rows: toPayload() })
...
async function submitRows() {
  ...
  try {
    ...
    await api.post('/departments/bulk/store', { rows: toPayload() })
    router.push({ name: 'master-departments' })
    toast.success(`${count} department${count === 1 ? '' : 's'} imported successfully.`)
  } catch (err) {
    ...
    modalError.value = data?.message || 'Could not import these departments.'
  } finally {
...
<template>
  <div>
    <div class="max-w-[900px] mx-auto">
      ...
      <div class="flex items-start justify-between gap-3 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-[20px] sm:text-[24px] font-semibold text-brand leading-tight">Bulk Upload Departments</h1>
            <p class="mt-1 text-[13px] sm:text-sm text-muted">Create many departments at once from a CSV file.</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div class="space-y-4">
        <section class="bg-white rounded-[28px] shadow-card p-4 sm:p-5">
          ...
          <p class="text-[13px] text-muted mb-3">
            Columns: <span class="font-medium text-gray-700">Name, Code</span>. Name is required and must be unique; Code is optional.
          </p>
          <button
            type="button"
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" @click.self="closeModal">
      <div class="bg-white rounded-[24px] shadow-card w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
        <div class="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-soft">
          <div>
            <h2 class="text-lg font-bold text-black">Review Uploaded Departments</h2>
            ...
            <table class="w-full min-w-[420px] text-left border-separate border-spacing-0">
              ...
              <tbody class="bg-white">
                <tr v-for="(row, index) in rows" :key="index" class="align-top text-[12px] even:bg-gray-50">
                  ...
                  <td class="px-2 py-1.5">
                    <input
                      :ref="(el) => setFieldRef(index, 'code', el)"
                      v-model="row.code"
                      type="text"
                      class="w-full h-8 px-2 rounded-lg bg-input-bg text-[12px] outline-none border"

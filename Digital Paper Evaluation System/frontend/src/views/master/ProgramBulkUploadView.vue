...
// the Teacher bulk upload and the single Add/Edit Program form).
...
function downloadTemplate() {
  ...
  XLSX.writeFile(workbook, 'program-bulk-upload-template.csv', { bookType: 'csv' })
}
...
async function runValidate() {
  ...
  modalError.value = ''
  try {
    const res = await api.post('/programs/bulk/validate', { rows: toPayload() })
...
async function submitRows() {
  ...
  try {
    ...
    modalError.value = data?.message || 'Could not import these programs.'
  } finally {
    ...
    <div class="max-w-[1000px] mx-auto">
      ...
      <div class="flex items-start justify-between gap-3 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-[20px] sm:text-[24px] font-semibold text-brand leading-tight">Bulk Upload Programs</h1>
            <p class="mt-1 text-[13px] sm:text-sm text-muted">Create many programs at once from a CSV file.</p>
          </div>
        </div>
        <button
          type="button"

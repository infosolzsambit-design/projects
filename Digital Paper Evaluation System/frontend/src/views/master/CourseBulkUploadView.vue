...
function downloadTemplate() {
  ...
  XLSX.writeFile(workbook, 'course-bulk-upload-template.csv', { bookType: 'csv' })
}
...
async function runValidate() {
  ...
  modalError.value = ''
  try {
    const res = await api.post('/courses/bulk/validate', { rows: toPayload() })
...
async function submitRows() {
  ...
  try {
    ...
    await api.post('/courses/bulk/store', { rows: toPayload() })
    router.push({ name: 'master-courses' })
    toast.success(`${count} course${count === 1 ? '' : 's'} imported successfully.`)
  } catch (err) {
    ...
    modalError.value = data?.message || 'Could not import these courses.'
  } finally {
    ...
    <div class="max-w-[900px] mx-auto">
      ...
      <div class="flex items-start justify-between gap-3 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-[20px] sm:text-[24px] font-semibold text-brand leading-tight">Bulk Upload Courses</h1>
            <p class="mt-1 text-[13px] sm:text-sm text-muted">Create many courses at once from a CSV file.</p>
          </div>
        </div>
        <button
          type="button"
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4" @click.self="closeModal">
      <div class="bg-white rounded-[24px] shadow-card w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
        <div class="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-soft">
          <div>
            <h2 class="text-lg font-bold text-black">Review Uploaded Courses</h2>

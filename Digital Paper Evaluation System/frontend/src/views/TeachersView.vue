...
onMounted(async () => {
  ...
  if (authStore.can('teacher-list')) fetchTeachers(1)
})
        ...
        <table class="w-full min-w-[1150px] text-left">
          <thead>
            <tr class="bg-subject-header text-white text-[13px] font-medium">
              ...
              <th class="px-4 py-3.5 font-medium" v-if="authStore.can('teacher-face-scan')">Face Scan</th>
              <th class="px-4 py-3.5 font-medium" v-if="authStore.can('teacher-status-change')">Status</th>
              <th class="px-4 py-3.5 font-medium text-center rounded-tr-2xl" v-if="authStore.can('teacher-edit') || authStore.can('teacher-delete')">Action</th>
            </tr>
          </thead>
          <tbody class="bg-white">
            ...
            >
              ...
              <td class="px-4 py-3.5">{{ teacher.email }}</td>
              <td class="px-4 py-3.5">{{ teacher.phone_no || '—' }}</td>
              <td class="px-4 py-3.5">{{ teacher.department || '—' }}</td>
              <td class="px-4 py-3.5">{{ teacher.designation || '—' }}</td>
              <td v-if="authStore.can('teacher-face-scan')" class="px-4 py-3.5" @click.stop>
                <button
              ...
              <td v-if="authStore.can('teacher-status-change')" class="px-4 py-3.5" @click.stop>
                <button
                  type="button"
                  role="switch"
                  ...
                  :class="teacher.is_active ? 'bg-btn-gradient text-white justify-start pl-3 pr-7' : 'bg-gray-300 text-gray-600 justify-end pl-7 pr-3'"
                ...
                >
                  ...
                  <span
                    ...
                    :class="teacher.is_active ? 'translate-x-16' : 'translate-x-0'"
                  ></span>
                </button>
              </td>
              <td v-if="authStore.can('teacher-edit') || authStore.can('teacher-delete') || authStore.can('teacher-face-scan-applicable')" class="px-4 py-3.5 text-center relative" @click.stop>

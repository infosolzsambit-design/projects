...
const FIELD_ORDER = ['name', 'sort_order']
const fieldRefs = {
  name: ref(null),
  sort_order: ref(null),
}
...
// whatever was actually loaded for edit (captured once loadGroup() finishes
// below). Not reactive on purpose — it's a fixed target, not something that
// should track the form's own live edits.
...
async function loadGroup() {
  ...
  loadError.value = ''
  try {
    const res = await api.get(`/permission-groups/${groupId.value}`)
    const group = res.data.data
    form.name = group.name
    form.sort_order = group.sort_order
    initialForm = { ...form }
  } catch (err) {
    loadError.value = err.response?.data?.message || 'Could not load this permission group.'
  } finally {
    loading.value = false
  }
}
...
function applyServerErrors(err) {
  ...
  formError.value = data?.message || 'Could not save permission group.'
}
...
async function submitForm() {
  ...
  try {
    ...
    if (isEdit.value) {
      await api.put(`/permission-groups/${groupId.value}`, payload)
    } else {
      await api.post('/permission-groups', payload)
    }
    router.push({ name: 'configurations-permission-groups' })
    toast.success(isEdit.value ? 'Permission group updated successfully.' : 'Permission group created successfully.')
  } catch (err) {
    ...
    <div class="max-w-[1100px] mx-auto">
      ...
      <div class="flex items-start justify-between gap-3 mb-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="text-[20px] sm:text-[24px] font-semibold text-brand leading-tight">{{ isEdit ? 'Edit Permission Group' : 'Add Permission Group' }}</h1>
            <p class="mt-1 text-[13px] sm:text-sm text-muted">
              {{ isEdit ? "Update this permission group's details." : 'Create a new permission group for the system.' }}
            </p>
          </div>
        </div>
        <button
      ...
      <form v-else class="space-y-4" @submit.prevent="submitForm">
        <section class="bg-white rounded-[28px] shadow-card p-4 sm:p-5">
          <h2 class="text-[16px] sm:text-[18px] font-semibold text-gray-900 mb-1">Permission Group Details</h2>
          <p class="text-[13px] text-muted mb-3">Name is required and must be unique. Sort order controls display position (lower shows first).</p>
        ...
        <div class="flex flex-wrap gap-3 justify-center sm:justify-end pb-2">
          ...
          <button
            ...
            :disabled="saving"
          >
            {{ saving ? 'Saving…' : isEdit ? 'Save Changes' : 'Create Permission Group' }}
          </button>
        </div>
      </form>
    </div>

...
async function loadRoles() {
  try {
    // RoleController::index has no unpaginated "all" mode — capped at
    // config('pagination.max_per_page'), which comfortably covers this
    // app's small, hand-managed set of roles.
    const res = await api.get('/roles', { params: { per_page: 30 } })
    availableRoles.value = res.data.data.items
  } catch {
    // Non-fatal — the role filter just stays empty; search/status still work.
  }
}
        ...
        <div class="relative">
          ...
          >
            <div class="flex flex-col gap-3">
              ...
              <div class="flex flex-col gap-1">
                <label class="text-[13px] text-label">Role</label>
                <div class="relative">
                  <select
                    v-model="roleFilter"
                    class="appearance-none w-full h-8 pl-3 pr-9 rounded-xl bg-input-bg text-[13px] text-gray-800 outline-none border border-input-border focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/15 transition cursor-pointer"
        ...
        <table class="w-full min-w-[780px] text-left">
          ...
          <tbody class="bg-white">
            ...
            >
              ...
              <td class="px-4 py-1 font-semibold">{{ user.name }}</td>
              <td class="px-4 py-1">{{ user.username }}</td>
              <td class="px-4 py-1">{{ user.email }}</td>
              <td class="px-4 py-1">{{ (user.roles || []).map((r) => r.name).join(', ') || '—' }}</td>
              <td v-if="authStore.can('user-status-change')" class="px-4 py-1" @click.stop>
                <button
                  type="button"
                  role="switch"
                  ...
                  :class="user.is_active ? 'bg-btn-gradient text-white justify-start pl-2.5 pr-6' : 'bg-gray-300 text-gray-600 justify-end pl-6 pr-2.5'"
                  @click="toggleStatus(user)"
                >
                  <span>{{ user.is_active ? 'Active' : 'Inactive' }}</span>
                  <span
                    class="absolute top-[5px] left-[5px] w-[14px] h-[14px] rounded-full bg-white shadow transition-transform duration-200"
                    :class="user.is_active ? 'translate-x-[56px]' : 'translate-x-0'"
                  ></span>
                </button>
              </td>
              <td v-if="authStore.can('user-edit') || authStore.can('user-delete')" class="px-4 py-1 text-center relative" @click.stop>
                <RowActionMenu>
